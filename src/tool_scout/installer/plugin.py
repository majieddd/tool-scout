"""Strategy B — Claude Code plugin install.

Plugins live at ~/.claude/plugins/<plugin_name>/. Same shape as skills:
copy local source dir to target, log, support uninstall.
"""
...
from tool_scout.installer.paths import PLUGINS_DIR
...
def install(tool: Tool, *, dry_run: bool = False) -> tuple[bool, dict]:
    ...
    target = PLUGINS_DIR / _slug(tool)
    diff = {"source": str(src), "target": str(target), "exists_before": target.exists()}
    if dry_run:
        write_audit("install", tool.id, strategy="plugin_dry_run", diff=diff)
        return True, diff
    if target.exists():
        shutil.rmtree(target)
    target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copytree(src, target) if src.is_dir() else shutil.copy2(src, target)
    write_audit("install", tool.id, strategy="plugin", diff=diff)
    record_install(tool.id, strategy="plugin", target_path=str(target), config_diff=diff)
    return True, diff
...
def uninstall(tool: Tool) -> tuple[bool, dict]:
    ...
    if target.exists():
        shutil.rmtree(target)
    write_audit("uninstall", tool.id, strategy="plugin", diff=diff)
    record_install(tool.id, strategy="plugin_uninstall", target_path=str(target), config_diff=diff)
    return True, diff
